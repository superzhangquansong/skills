...
# 1. 强制认证与安全约束
- **智能预检**: 执行业务前先检查 Token。若失效，优先调用 `refreshToken` 实现无感续期。
- **任务连续性 (Task Continuity)**: 登录成功后，AI 必须**自动记忆并恢复**登录前被中断的原始任务。
- **分步向导**: 必须按“用户名 -> 密码”的顺序进行询问，不得要求用户一次性提供。
- **重试逻辑**: 认证失败时，AI 需告知具体原因，并询问是否重新开始认证流程。
- **安全红线**: 严禁在任何日志或响应中展示用户的明文密码。
...
### 场景 A：任务恢复型登录 (Context Recovery)
1. **用户**: “帮我查询方悦面板并把第 1 个加入购物车。”
2. **AI**: 检测到无 Token -> “🔑 **HDL 认证** - 请先提供您的 **用户名**。”
3. **用户**: “test_user”
4. **AI**: “好的。现在请提供 **登录密码**。”
5. **用户**: “******”
6. **AI**: 认证成功 -> “登录成功！**已为您恢复任务**：方悦面板查询中... 找到 5 个产品。已成功将第 1 个（ERP: 308013573）加入购物车。✅”

### 场景 B：静默刷新 (Silent Refresh)
1. **AI**: 执行查询请求 -> 返回 401。
2. **AI**: (后台执行 `refreshToken`) -> 刷新成功 -> (重新执行查询) -> **直接返回查询结果**。
3. **结果**: 用户全程无感知。

### 场景 C：登录失败循环
1. **AI**: 登录返回“密码错误”。
2. **AI**: “❌ **认证失败**：用户名或密码不正确。需要重新开始登录吗？请输入您的 **用户名**：”。
