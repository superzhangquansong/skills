...
4. **二维码 H5 登录向导 (QR-H5-WebSocket Login Wizard)**: 
   - **通信规范**:
     - **H5 访问地址**: `http://14.103.240.202:8588/ClawLogin.html?sessionId={sessionId}`
     - **WebSocket 服务端**: `ws://14.103.240.202:8588/ws-openclaw-login`
     - **回传 JSON 格式**: `{"loginName": "...", "loginPwd": "...", "sessionId": "..."}`
   - **交互流程**:
     1. **生成二维码**: AI 主动生成一个带有随机 `sessionId` 的二维码，指向上述 H5 地址。
     2. **建立监听**: AI 通过 WebSocket 客户端连接到 `/ws-openclaw-login` 并订阅或识别 `sessionId` 频道。
     3. **H5 登录与推送**: 用户在 H5 页面点击登录，前端通过 WebSocket 发送凭据。
     4. **AI 接收并代理**: AI 收到消息后立即执行 `user-auth-api` 获取 Token。
     5. **重试机制**: 若认证失败，AI 告知用户并重新生成带时效的新二维码。
5. **任务中断与自动恢复 (Task Context Recovery)**: 
   - 登录成功后，AI 必须立即、自动地继续执行登录前被中断的原始任务，严禁要求用户再次发送指令。
6. **无感刷新 (Silent Refresh)**: 
   - 业务接口返回 401 时，立即调用 `refreshToken` 接口实现无感续期。
7. **主动退出与凭据清理**: 
   - 响应“退出登录”指令，清除 Token 并关闭当前安全会话。
8. **拒绝执行**: 只要 Token 验证未成功且无法自动恢复，严禁继续执行后续业务。
