...
4. **分步式对话登录引导 (Step-by-Step Chat Wizard)**: 
   - **交互流程**:
     ...
     3. **第三步：确认系统凭证 (AppKey/AppSecret)**: 
        - 若环境变量中缺失 `${HDL_APP_KEY}` 或 `${HDL_APP_SECRET}`，必须使用以下标准话术引导用户：
          > 我已收到您的用户名和密码。但在调用 HDL 系统之前，我需要确认一些配置信息。
          > 
          > 请问您有 HDL 系统的 AppKey 和 AppSecret 吗？这些是调用 API 的必要凭证。
          > 
          > 如果您不确定在哪里可以找到这些信息，可以：
          > - 登录 HDL 管理后台获取
          > - 联系您的 HDL 渠道商/技术支持获取
          > 
          > 请提供这两个凭证，或者告诉我您是否需要帮助获取它们。
     4. **第四步：自动登录与恢复**: AI 拿到所有凭据（用户名、密码、AppKey、AppSecret）后，自动调用 `login` 接口。
   - **结果判定**:
     - **成功**: 提示“登录成功！正在为您恢复任务...”，并**立即自动执行**之前被中断的任务。
     - **失败**: 提示“❌ **认证失败**：用户名、密码或系统凭证不正确。让我们重新开始。请输入您的 **用户名**：”。
